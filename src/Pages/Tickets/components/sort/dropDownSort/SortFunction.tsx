...
export const SortFunction= ({ sort, setSortingData, sortingData, dataDefault } : ISortProps) => {
        // sort by Details
      ...
      if(sort.prop === 'Name' && sort.direction === 'desc'){
        const newData = [...sortingData];
        const sortNewData = newData.sort((a, b) => (b.username.toLowerCase() > a.username.toLowerCase() ? 1 : -1));
        setSortingData(sortNewData);
        
        
      }else if(sort.prop === 'Name' && sort.direction === 'asc'){
        const newData = [...sortingData];
        const sortNewData = newData.sort((a, b) => (a.username.toLowerCase() > b.username.toLowerCase() ? 1 : -1));
        setSortingData(sortNewData);
      }
      ...
      if(sort.prop === 'Date' && sort.direction === 'desc'){
        const newData = [...sortingData];
        const sortNewData = newData.sort((a, b) => (Date.parse(a.registeredAt) > Date.parse(b.registeredAt) ? 1 : -1));
        setSortingData(sortNewData);
      }else if(sort.prop === 'Date' && sort.direction === 'asc'){
        const newData = [...sortingData];
        const sortNewData = newData.sort((a, b) => (Date.parse(b.registeredAt) > Date.parse(a.registeredAt) ? 1 : -1));
        setSortingData(sortNewData);
      }
